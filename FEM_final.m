%% Data Import
nodes = xlsread('nodesreduced.xlsx');
triangles = xlsread('trianglesreduced.xlsx');
edge = xlsread('edgesreduced.xlsx');
%% Assumptions
L = 1;
H = 0.05;
C = 1;
...
for  i = 1:length(edge(:,1))
    ...
    K4 = K4./(4*A^2);
    F_without_phi = 2*K4;
    phi_incident = [(H*0.5/T)*exp(-1i*k*(x1*cos(direction)+y1*sin(direction))*exp(-1i*w*t)); (H*0.5/T)*exp(-1i*k*(x2*cos(direction)+y2*sin(direction))*exp(-1i*w*t))];
    F_final = F_without_phi*phi_incident;
...
toc
%% Shoreline integral
edge_shore_integral = xlsread('edgesshore_integral.xlsx');
alpha = 0.3;
syms y
for  i = 1:length(edge_shore_integral(:,1))
    y1 = nodes(edge_shore_integral(i,1),3);
    y2 = nodes(edge_shore_integral(i,2),3);
    length_element = abs(y2-y1);
    % C = sqrt((9.81/k)*tanh(k*((x1+x2)/2)));
    % Cg = 0.5*(1+((2*k*((x1+x2)/2))/(sinh(2*k*((x1+x2)/2)))));
    Ni = 1-(y/length_element);
    Nj = y/length_element;
    N = [Ni Nj];
    K3 = -1i*k*alpha*C*Cg*int(N'*N,y,y2,y1);
    K3 = double(K3);
    K(edge_shore_integral(i,1),edge_shore_integral(i,1)) = K(edge_shore_integral(i,1),edge_shore_integral(i,1)) + K3(1,1);
    K(edge_shore_integral(i,1),edge_shore_integral(i,2)) = K(edge_shore_integral(i,1),edge_shore_integral(i,2)) + K3(1,2);
    K(edge_shore_integral(i,2),edge_shore_integral(i,1)) = K(edge_shore_integral(i,2),edge_shore_integral(i,1)) + K3(2,1);
    K(edge_shore_integral(i,2),edge_shore_integral(i,2)) = K(edge_shore_integral(i,2),edge_shore_integral(i,2)) + K3(2,2);
end

%% Load Shore data (Run from this)
edge_shore = xlsread('edgesshore.xlsx');
K(edge_shore(:,1)',:) = [];
K(:,edge_shore(:,1)') = [];
F(edge_shore(:,1)',:) = [];
%% Solve
...
% for i = 1:length(triangles(:,1))
%     for j=1:3
%        x(i,j) = nodes(triangles(i,j+1),2);
%        y(i,j) = nodes(triangles(i,j+1),3);
%        z(i,j) = Z(triangles(i,j+1),1);
%     end
% end
scatter3(X, Y, eta_phase_radian,'b')
% scatter3(X, Y, etamagnitude_all(:,2),'b')
% hold on
% scatter3(X, Y, etamagnitude_all(:,1),'r')
shading interp
%%
etamagnitude_all(:,2) = etamagnitude(:,1);
% plot3(x,y,etamagnitude,'o')
